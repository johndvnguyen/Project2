...
void nearest(struct kdNode *root, struct kdNode *nd, int i, int dim,
        airportList *best, double *best_dist,int k)
{
    double d, dx, dx2;
    ...
    nearest(dx > 0 ? root->left : root->right, nd, i, dim, best, best_dist,k);
    if (dx2 >= *best_dist) return;
    nearest(dx > 0 ? root->right : root->left, nd, i, dim, best, best_dist,k);
}
...
nearest(tree,searchNode, 0 , 2, &found, &best_dist,5);
